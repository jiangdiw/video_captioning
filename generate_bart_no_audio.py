"""
Generate captions using BartCaptioningModelNoAudio (CLIP + DINOv2 only).

Usage:
    python generate_bart_no_audio.py
    python generate_bart_no_audio.py --n 20 --num-beams 4
"""
...
def parse_args():
    ...
    p.add_argument("--checkpoint",   default="outputs/bart_no_audio_checkpoints/best.pt")
...
def main():
    ...
    for vid in sample:
        ...
        with torch.no_grad():
            token_ids = model.generate(clip, dino, max_new_tokens=40, num_beams=args.num_beams)
        generated = tokenizer.decode(token_ids[0], skip_special_tokens=True)

        print(f"[{vid}]")
        print(f"  Generated : {generated}")
        for i, c in enumerate(captions[vid][:3]):
            print(f"  GT {i+1}      : {c}")
        print()
