"""
Training script for BartCaptioningModelNoAudio (CLIP + DINOv2 only).

Usage:
    python train_bart_no_audio.py --debug
    python train_bart_no_audio.py --epochs 20 --batch-size 16
    python train_bart_no_audio.py --freeze-bart --epochs 10
"""
...
class BartMSRVTTDatasetNoAudio(Dataset):
    def __init__(self, split, tokenizer, dataset_mode="subset",
                 max_caption_len=40, debug_n=None):
...
def parse_args():
    ...
    p.add_argument("--out-dir",     default="outputs/bart_no_audio_checkpoints")
    return p.parse_args()
...
def main():
    ...
    model = BartCaptioningModelNoAudio(freeze_bart=args.freeze_bart).to(device)
