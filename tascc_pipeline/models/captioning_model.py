...
from models.attention_lstm_decoder import AttentionLSTMDecoder
...
class PrecomputedCaptioningModel(nn.Module):
    ...
    def forward(self, embeddings, captions):
        seq = torch.tanh(self.encoder_proj(embeddings)).unsqueeze(1)
        init_h = seq.squeeze(1)
        return self.decoder(seq, captions, init_h)

    @torch.no_grad()
    def generate(self, embeddings, max_len=20, sos_idx=1, eos_idx=2):
        seq = torch.tanh(self.encoder_proj(embeddings)).unsqueeze(1)
        init_h = seq.squeeze(1)
        return self.decoder.generate(seq, init_h, max_len, sos_idx, eos_idx)


class SequencePrecomputedCaptioningModel(nn.Module):
    def __init__(self, seq_input_dim, pooled_input_dim, vocab_size,
                 embed_dim=256, hidden_dim=512, dropout=0.5):
        super().__init__()
        self.seq_input_dim = seq_input_dim
        self.pooled_input_dim = pooled_input_dim
        self.hidden_dim = hidden_dim
        self.embed_dim = embed_dim
        self.dropout_p = dropout
        self.seq_proj = nn.Linear(seq_input_dim, hidden_dim)
        self.init_proj = nn.Linear(pooled_input_dim, hidden_dim)
        self.decoder = AttentionLSTMDecoder(
            vocab_size=vocab_size,
            embed_dim=embed_dim,
            hidden_dim=hidden_dim,
    ...
    def forward(self, seq_embeddings, pooled_embeddings, captions):
        seq = torch.tanh(self.seq_proj(seq_embeddings))
        init_h = torch.tanh(self.init_proj(pooled_embeddings))
        return self.decoder(seq, captions, init_h)

    @torch.no_grad()
    def generate(self, seq_embeddings, pooled_embeddings, max_len=20, sos_idx=1, eos_idx=2):
        seq = torch.tanh(self.seq_proj(seq_embeddings))
        init_h = torch.tanh(self.init_proj(pooled_embeddings))
        return self.decoder.generate(seq, init_h, max_len, sos_idx, eos_idx)
