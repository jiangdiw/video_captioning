"""
BART captioning with audio — BART decoder frozen, trains encoder + proj + lm_head only.

Usage:
    python train_bart_frozen_decoder.py --debug
    python train_bart_frozen_decoder.py --epochs 20 --batch-size 16
"""
...
def freeze_decoder(model):
    ...
    for p in model.bart.model.decoder.parameters():
        p.requires_grad = False
    # lm_head, model.encoder (Approach1Encoder), model.proj stay trainable


class BartMSRVTTDataset(Dataset):
    def __init__(self, split, tokenizer, dataset_mode="subset",
                 audio_dir=None, debug_n=None):
    ...
    def __getitem__(self, idx):
        ...
        ap = self.audio_dir / f"{vid}.npy"
        audio = torch.tensor(np.load(ap), dtype=torch.float32) if ap.exists() else torch.zeros(1, 128)
...
def parse_args():
    ...
    p.add_argument("--out-dir",      default="outputs/bart_frozen_decoder_checkpoints")
    return p.parse_args()
...
def main():
    ...
    train_ds = BartMSRVTTDataset("train", tokenizer, args.dataset_mode, args.audio_dir, debug_n=debug_n)
    val_ds   = BartMSRVTTDataset("val",   tokenizer, args.dataset_mode, args.audio_dir, debug_n=debug_n)
