...
class COCOScorer(object):
    ...
    def score(self, GT, RES, IDs):
        ...
        for ID in IDs:
            ...
            res[ID] = RES[ID]
        print('tokenization...')
        gts = safe_tokenize(gts)
        res = safe_tokenize(res)
...
def tokenization_is_valid(original, tokenized):
    if len(original) != len(tokenized):
        return False
    for key, captions in original.items():
        if key not in tokenized:
            return False
        tokenized_captions = tokenized[key]
        if not isinstance(tokenized_captions, list):
            return False
        if len(tokenized_captions) != len(captions):
            return False
        if any(not isinstance(item, str) for item in tokenized_captions):
            return False
        if all(not item.strip() for item in tokenized_captions):
            return False
    return True
...
def safe_tokenize(data):
    try:
        ...
        if tokenization_is_valid(data, tokenized):
            return tokenized
        print('warning: PTBTokenizer returned incomplete results, using simple tokenizer')
    except Exception as exc:
        print(f'warning: PTBTokenizer failed ({exc}), using simple tokenizer')
